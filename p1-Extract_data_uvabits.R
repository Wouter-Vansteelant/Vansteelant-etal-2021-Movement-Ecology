...
############################################################
#####     Determine trips based on distance to island   ####
...
write.csv(data_trips_short,'./Data/EF_SummerShortTrips-v20210105.csv')
...
trip.lims
rm(out.ends,return.ends,trip.ends,out.starts,return.starts,trip.starts,trip.lims)

data$phase <- ifelse(data$trip == "out" & data$date >= as.Date(data$trip.start) & data$dt < data$trip.end,"out",
                    ifelse(data$trip == "return" & data$dt >= data$trip.start & data$dt < data$trip.end,"return",
...
write.csv(data,'./Data/EF_fulldata_migrants-AnnotatedCycle-v20210421.csv') 
length(unique(data$dev))

### Segment track in travel vs stationary parts
################################################################
data <- data[order(data$dev,data$dt),]
data$segment <- as.numeric(c(0,cumsum(as.logical(diff(as.numeric(as.factor(paste(data$segment,data$travel))))))))
...
write.csv(data,'./Data/EF_migration-v20210421.csv')
write.csv(ss2,'./Data/EF_SummerWinter-v20210421.csv')
rm(ss,ss2)
...
write.csv(segs,'./Data/EF_migration_stop-segments-v20210421.csv')
