robust.NCEP.interp.pressure.debugged <-
  function(variable, lat, lon, dt, pressure,
           reanalysis2=TRUE, interpolate.space=TRUE, interpolate.time=TRUE, keep.unpacking.info=TRUE, return.units=FALSE, interp='idw', p=1){
...
#lat = 20
#lon = -1.2
#dt = as.character(ss.return.2013$dt[1])
#reanalysis2=FALSE
...
for(i in 1:iterations){
  ...
  if(i == 1 | keep.unpacking.info == FALSE){
    ...
    fail <- 0
    while(trying.out != 0){
      trying.out <- try(download.file(paste("http://www.esrl.noaa.gov/psd/thredds/dodsC/Datasets/ncep.reanalysis",ifelse(reanalysis2[i] == TRUE, "2",""),"/pressure/",variable[i],".",year1,".nc.das", sep=''), mode="wb", method="libcurl", scale.offset.missingvals.temp), silent=TRUE)
      fail <- fail + 1
    ...
    ##
    add.offset <- if(reanalysis2[i] == TRUE & any(grepl('add_offset', readLines(scale.offset.missingvals.temp)))){as.numeric(strsplit(strsplit(grep('add_offset', x=readLines(scale.offset.missingvals.temp), value=TRUE, fixed=TRUE), ';')[[1]][1], 'add_offset ')[[1]][2]) } else { 0 }
    scale.factor <- if(reanalysis2[i] == TRUE & any(grepl('add_offset', readLines(scale.offset.missingvals.temp)))){as.numeric(strsplit(strsplit(grep('scale_factor', x=readLines(scale.offset.missingvals.temp), value=TRUE, fixed=TRUE), ';')[[1]][1], 'scale_factor ')[[1]][2]) } else { 1 }
